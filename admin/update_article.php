...
try {
    if ($image && $image['error'] === UPLOAD_ERR_OK) {
        ...
        $controller->updateBlogPost($id, $title,$content, $author, $featured,Constants::SITE_URL."\n/uploads\n/".$newImageName); 

        http_response_code(201);
        echo json_encode(['message' => 'Post Updated successfully', 'image_path' => $newImageName]);
        exit;
    } else {
        $controller->updateBlogPost($id, $title, $content, $author, $featured);
        http_response_code(201); 
        echo json_encode(['message' => 'Post Updated successfully', 'image_path' => null]);
        exit;
    }
} catch (\Throwable $th) {
    http_response_code(500);
