...
try {
    if ($image && $image['error'] === UPLOAD_ERR_OK) {
        ...
        $controller->createBlogPost($title,$content, $author, $_SESSION['user_id'], $featured,Constants::SITE_URL."\n/uploads\n/".$newImageName); 

        http_response_code(201);
        echo json_encode(['message' => 'Post created successfully', 'image_path' => $newImageName]);
        exit;
    } else {
        $controller->createBlogPost($title, $content, $author, $_SESSION['user_id'],$featured);
        http_response_code(201); 
