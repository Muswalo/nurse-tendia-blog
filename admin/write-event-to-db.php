...
try {
    if ($image && $image['error'] === UPLOAD_ERR_OK) {
        ...
        $controller->createEvent($title, $description, $date, $location, $_SESSION['user_id'], $featured, Constants::SITE_URL . "/uploads/" . $newImageName);

        http_response_code(201);
        echo json_encode(['message' => 'Event created successfully', 'image_path' => $newImageName]);
        exit;
    } else {
        $controller->createEvent($title, $description, $date, $location, $_SESSION['user_id'], $featured);
        http_response_code(201);
        echo json_encode(['message' => 'Event created successfully', 'image_path' => null]);
        exit;
    }
} catch (\Throwable $th) {
    http_response_code(500);
